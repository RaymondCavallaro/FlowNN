...
export class PressureField {
  ...
  solveFlow({ includeTrainingOnly = false } = {}) {
    this.clearVectors();

    for (let index = 0; index < this.valves.length; index += 1) {
      if (!includeTrainingOnly && this.trainingOnly[index]) continue;
...
export class PressureNetwork {
  ...
  step({
    ...
    this.field.solveFlow();
    this.field.applyFlow({
      learning,
      learnValve: (valve, from, to, throughput) => {
        this.learnValve(valve, from, to, throughput, learningRate, teacherOutputId);
      },
    });
