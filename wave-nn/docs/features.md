...
| Flow-replacement probes | Direct handles are tested against possible flow-based replacements before being treated as core mechanisms. | `drain route does not replace direct decay yet`; `budgeted flow lets large outward route act as drain`; `resistance extremes simulate connection availability`; `coactivation carves route support for later flow` |
